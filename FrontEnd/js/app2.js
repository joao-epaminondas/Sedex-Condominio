...
function filterData(searchTerm) {
  ...
  var filteredData = data.filter(function(record) {
    ...
    return (
      descricaoMatch ||
      destinatarioMatch ||
      (enderecoCompleto.includes(searchTerm) && !startsWithNumber) ||
      (startsWithNumber ? record.numeroSedex === searchTerm : false) ||
      dataMatch
    );
  });
...
function displayModalData(record) {
      $("#modalimg2").attr("src", "#");
      var caminhoRelativo = record.url_img_assinatura.replace('C:/wamp64/www/Projeto Sedex/FrontEnd', '../FrontEnd');
