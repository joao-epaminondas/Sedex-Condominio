...
function salvarDadosNoServidor(data, successCallback) {
   $.ajax({
      url: '../BackEnd/php/workspace/app1.php',
      type: 'POST',
      data: data,
...
function listarDadosDoServidor(paginaAtual) {
   ...
   if (isMobileDevice()) {
      ...
      theadRow.append($("<th>").html("Nº Sedex"), $("<th>").html("Destinatário"), $("<th>").html("Endereço"), $("<th>").html("Ações") // For buttons on PC
      );
   }
   ...
   for (var i = startIndex; i < endIndex; i++) {
      ...
      var row = $("<tr>").addClass("dinamica").attr("id", "sedex_" + record.numeroSedex).attr("data-toggle", "modal").attr("data-target", "#myModal3").attr("data-destinatario", record.destinatario).attr("data-rua", record.rua).attr("data-numeroRua", record.numerorua).attr("data-cxp", record.cxp).attr("data-numeroSedex", record.numeroSedex).attr("data-descricao", record.descricao).attr("data-entrada_funcionario", record.entrada_funcionario).attr("data-saida_funcionario", record.saida_funcionario).attr("data-data_entrada", record.data_entrada).attr("data-hora_entrada", record.hora_entrada).attr("data-data_saida", record.data_saida).attr("data-hora_saida", record.hora_saida).attr("data-retirado", record.retirado).attr("data-img", record.url_img_assinatura);
      if (isMobileDevice()) {
         mobile(row, record);
      } else {
         pc(row, record);
   ...
   var tfoot = $("<tfoot>").append($("<tr>").append($("<td>").attr("colspan", isMobileDevice() ? 2 : 4).append(generatePaginationButtons(totalPages, currentPage))));
   table.append(tfoot);
...
function generatePaginationButtons(totalPages, currentPage) {
   ...
   for (var i = 1; i <= totalPages; i++) {
      var pageButton = $("<button>").text(i).addClass("btn btn-pagination").toggleClass("active", i === currentPage).on("click", function() {
...
function pc(row, record) {
   row.append($("<td>").text(record.numeroSedex), $("<td>").text(reduzirTextoDestinatario(record.destinatario, 25)), $("<td>").text(record.rua + ", " + record.numerorua), $("<td>").html("<button class='btn btn-info btn-sm' data-toggle='modal' data-target='#myModal3' data-destinatario='" + record.destinatario + "' data-rua='" + record.rua + "' data-numeroRua='" + record.numerorua + "' data-cxp='" + record.cxp + "' data-numeroSedex='" + record.numeroSedex + "' data-descricao='" + record.descricao + "' data-entrada_funcionario='" + record.entrada_funcionario + "' data-data_entrada='" + record.data_entrada + "' data-hora_entrada='" + record.hora_entrada + "' data-retirado='" + record.retirado + "' data-img='" + record.url_img_assinatura + "'>Visualizar <i class='fas fa-eye'></i></button>" + " <span class='btn-separator'></span> " + // Add a separator or space
